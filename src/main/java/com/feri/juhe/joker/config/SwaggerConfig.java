...
public class SwaggerConfig {
    private ApiInfo createAI(){
        return new ApiInfoBuilder().title("笑话大全接口平台").description("基于SpringBoot+MybatisPlus实现的开心笑一笑").version("0.1")
    ...
    public Docket createD(){
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(createAI()).select().apis
                (RequestHandlerSelectors.basePackage("com.feri.juhe.joker.controller")).build();
    }
}
